...
function Navbar({navFunction}) {
  ...
  return(  
      <ul className="flex flex-col w-[60dvw] lg:flex-row lg:static lg:w-auto lg:bg-none justify-center lg:items-center items-end gap-5 md:gap-2">
        {links.map((link, i) => (
          <li key={i}><Link to={link.path} onClick={ navFunction }  className="px-4 py-2 text-white text-center  text-2xl rounded-md border-2 border-transparent hover:border-purple-600 hover:text-purple-600 ease-in duration-300 lg:text-base">
            {t(link.name)}
          </Link></li>
            ))}
